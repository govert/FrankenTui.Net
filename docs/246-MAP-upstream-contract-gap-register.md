...
| `GAP-344-WASM-SHOWCASE-RUNNER` | `partial` | `crates/ftui-showcase-wasm/src/runner_core.rs`, `wasm.rs`, and pane interruption/touch commits in `2d25a03d..40c98246` | Local web/wasm alignment now has a `ShowcaseRunnerCore` analogue that owns frame/size/scenario/screen state over the shared showcase web rendering path, steps the shared model with queued numbered-screen key/mouse input, nonzero `EventsProcessed` reporting for accepted host inputs/resizes, an upstream-shaped `fnv1a64:` patch hash, patch stats fields, flat patch `{ cells, spans }` buffers, prepared-buffer pointer/length analogues, wasm-style time controls, runner lifecycle pane dispatch logs, pane workspace export/import/generation/saved/dirty/layout-mode stubs, `Destroy()` no-op, and a complete pane pointer lifecycle (down, capture, leave before/after capture, blur, visibility-hidden, lost-pointer-capture, context-lost, render-stalled, native-touch). Numbered-screen mode renders through `ShowcasePage.RenderScreen` via `ShowcaseViewFactory` with screen metadata. It is still not a full wasm-bindgen `AppModel` runner, and terminal/web hosts are not yet unified on one app model. | Complete `364-DEM-H` by adapting terminal and web hosts to the same showcase model and adding broader screen selection/interruption evidence. |
