...
| OpenTUI migration semantic contract | `docs/spec/opentui-semantic-equivalence-contract.md`, `docs/spec/opentui-transformation-policy-matrix.md` | `partial` | Local loaders/validators now consume the upstream semantic and transformation-policy JSON contracts, doctor also persists the upstream confidence/licensing contracts, and a local OpenTUI contract gate now evaluates evidence manifests into clause-level results, but no translator/planner emits findings into that gate yet. | Tooling / doctor |
