...
| 10 | Advanced | `advanced_features` | `screens/advanced_features.rs` | Core | `ShowcaseSurface.BuildAdvancedFeatures` | partial: local two-pane patterns/composite surface with frame-hit regions for both panes plus matching mouse evidence plus mouse-mutated pattern/composite selection rendering; richer upstream advanced-pattern workflows and exact styling remain open |
