...
| `windows` | `conpty` | `validated-ci` | `headless`, `ci-windows`, `ci-windows-doctor`, `ci-windows-inline-transcripts` | no local ConPTY execution evidence is produced from this workspace; current Windows CI evidence is a redirected runner console with blank `WT_SESSION`, `ConPty`, and `TERM` values rather than a captured interactive ConPTY session; PTY transcript assertions remain Unix-only; the remaining blocker is tracked in [2026-03-12-windows-conpty-evidence-blocker.md](./2026-03-12-windows-conpty-evidence-blocker.md) | prefer the Windows console capability set and avoid Unix-only toggles unless explicitly supported |
