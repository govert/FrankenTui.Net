...
| --- | --- | --- | --- | --- | --- |
| `linux` | `unix-tty` | `validated-local` | `headless`, `pty`, `doctor`, `ci-linux` | raw-mode ownership stays at the managed backend boundary; transcript evidence is stronger than raw TTY state capture | prefer modern ANSI features and degrade cleanly when terminal metadata is sparse |
