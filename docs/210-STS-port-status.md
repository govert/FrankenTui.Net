...
- showcase hit-registry slice after upstream `f958e59e`
  ...
  mouse-mutated pane/scroll/wrap rendering, the Table Theme increment adds
  ...
  Focused showcase-shell verification is now `240` tests via
  `dotnet test tests/FrankenTui.Tests.Headless/FrankenTui.Tests.Headless.csproj --no-restore --filter FullyQualifiedName~ShowcaseShellTests`.
  Current local verification is `494` headless tests, `9` web tests, and `7`
  PTY tests via `dotnet test FrankenTui.Net.sln --no-restore`.
