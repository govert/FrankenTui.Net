...
- demo control-plane launch baseline after showcase inventory
  ...
  The Widget Gallery screen now has local progress, list, tabs, and table
  panels with frame-registered regions, matching mouse evidence, and
  mouse-mutated list/tab/table selections; richer upstream gallery state and
  exact styling remain tracked under `364-DEM`.
  ...
  navigation status, and frame-registered list, selected-row, stats, and footer
  ...
  layout, with frame-registered header, library, viewport, controls, metrics,
  and status-log regions plus matching mouse evidence; full upstream IR
  renderer, search/inspect modes, debug overlays, node navigation, minimap,
  palette rendering, exact JSONL schema, stateful mouse mutation, and full
...
- showcase hit-registry slice after upstream `f958e59e`
  ...
  frame-hit registry lane. The follow-on File Browser increment adds
  ...
  evidence, and the Form Validation increment adds mode, field, touched/dirty,
  error-summary, rules, controls, notifications, and diagnostics regions plus
  ...
  matching mouse evidence. The Performance Challenge increment adds header,
  metrics, sparkline, JSONL evidence, render-budget, stress-harness,
  degradation-tier, and footer-control regions plus matching mouse evidence,
  and the Explainability Cockpit increment adds header, diff-strategy,
  resize-regime, budget-decision, timeline, and source/control regions plus
  matching mouse evidence. The i18n Stress Lab increment adds locale-bar,
  string-lookup, plural-rules, RTL-layout, stress-lab, and footer-control
  regions plus matching mouse evidence. The VOI Overlay increment adds header,
  decision, posterior, observation, ledger, controls, and footer regions plus
  matching mouse evidence, and the Inline Mode Story increment adds header,
  ...
  routing priority, the Dashboard increment adds mouse-mutated overview/highlight/context rendering,
  the Shakespeare increment adds mouse-mutated focus/query/scroll/context rendering,
  ...
  mouse-mutated virtual-list item/focus/context rendering, the Markdown increment adds
  mouse-mutated pane/scroll/wrap/context rendering, the Data Viz increment adds
  ...
  increment adds mouse-mutated filter/event/detail/focus/context rendering, the Intrinsic Sizing
  ...
  the Inline Mode Story increment adds mouse-mutated compare/pause/upstream rate-wheel/height/scroll rendering,
  ...
  the Mermaid Showcase increment adds mouse-mutated focus/sample/zoom/panel/status/context rendering,
  the Mermaid Mega increment adds mouse-mutated focus/sample/zoom/detail/panel/context rendering,
  the Visual Effects increment adds mouse-mutated focus/effect/harness-scroll rendering,
  the Responsive Layout increment adds mouse-mutated focus/breakpoint/width/aside rendering,
  the Log Search increment adds mouse-mutated result/diagnostic/scroll/focus/pause rendering,
  the Live Markdown Editor increment adds mouse-mutated focus/search/cursor/diff/preview rendering,
  the Drag & Drop Lab increment adds mouse-mutated mode/list/selection/context/keyboard rendering,
  the Quake increment adds mouse-mutated focus/quality/yaw/pitch/fire/reset rendering, the Table Theme
  ...
  Focused showcase-shell verification is now `310` tests via
  `dotnet test tests/FrankenTui.Tests.Headless/FrankenTui.Tests.Headless.csproj --no-restore --filter FullyQualifiedName~ShowcaseShellTests`.
  Current local verification is `564` headless tests, `9` web tests, and `7`
  PTY tests via `dotnet test FrankenTui.Net.sln --no-restore`.
