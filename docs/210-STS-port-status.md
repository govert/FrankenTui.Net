...
- showcase hit-registry slice after upstream `f958e59e`
  ...
  mouse-mutated metric/narrative selection rendering, the Table Theme increment
  adds mouse-selected active preset mutation/rendering, the Terminal Capabilities
  ...
  Focused showcase-shell verification is now `242` tests via
  `dotnet test tests/FrankenTui.Tests.Headless/FrankenTui.Tests.Headless.csproj --no-restore --filter FullyQualifiedName~ShowcaseShellTests`.
  Current local verification is `496` headless tests, `9` web tests, and `7`
  PTY tests via `dotnet test FrankenTui.Net.sln --no-restore`.
