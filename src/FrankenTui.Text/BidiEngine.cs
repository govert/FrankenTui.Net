...
// UAX #9 engine adapted from SixLabors.Fonts v1.0.0, commit
// 32bef42997adb10268369ca149777f00e4241ce9 (Apache-2.0),
// with minimal local array-slice helpers. Unicode property data is supplied
// separately by the FrankenTui Unicode 16.0 adapter in Bidi.cs.

using System.Runtime.CompilerServices;

namespace FrankenTui.Text.BidiInternals
{
    internal struct ArrayBuilder<T>
    {
        ...
        public int Length
        {
            ...
            set
            {
                ...
                if (value > this.length)
                    Array.Clear(this.buffer!, this.length, value - this.length);
                this.length = value;
            }
        }
        ...
        public ref T this[int index]
        {
            get
            {
                if ((uint)index >= (uint)this.length) throw new ArgumentOutOfRangeException(nameof(index));
        ...
        public ArraySlice<T> Add(int count)
        {
            if (count < 0) throw new ArgumentOutOfRangeException(nameof(count));
            int start = this.length;
            this.Length = checked(this.length + count);
            return new ArraySlice<T>(this.buffer!, start, count);
        }

        public ArraySlice<T> Add(ReadOnlyArraySlice<T> source)
        {
            ArraySlice<T> destination = this.Add(source.Length);
            for (int i = 0; i < source.Length; i++) destination[i] = source[i];
            return destination;
        }

        public ArraySlice<T> AsSlice() => this.AsSlice(0, this.length);

        public ArraySlice<T> AsSlice(int start, int count)
        {
            if (start < 0 || count < 0 || start > this.length - count)
                throw new ArgumentOutOfRangeException(nameof(start));
            return new ArraySlice<T>(this.buffer ?? Array.Empty<T>(), start, count);
        }

        private void EnsureCapacity(int capacity)
        {
            if (this.buffer is not null && this.buffer.Length >= capacity) return;
            int newCapacity = Math.Max(capacity, this.buffer is null ? 8 : checked(this.buffer.Length * 2));
    ...
    internal readonly struct ReadOnlyArraySlice<T>
    {
        ...
        public ReadOnlyArraySlice(T[] array) : this(array, 0, array.Length) { }

        public ReadOnlyArraySlice(T[] array, int start, int length)
        {
        ...
        public ReadOnlyArraySlice<T> Slice(int start, int length)
            => new(this.array, this.start + start, length);

        public static implicit operator ReadOnlyArraySlice<T>(T[] array) => new(array);
    }
    ...
    internal readonly struct ArraySlice<T>
    {
        ...
        {
            for (int i = 0; i < this.Length; i++) this[i] = value;
        }

        public static implicit operator ReadOnlyArraySlice<T>(ArraySlice<T> slice)
            => new(slice.array, slice.start, slice.Length);

        public static implicit operator ArraySlice<T>(T[] array) => new(array);
    }

    internal readonly struct MappedArraySlice<T>
    {
        private readonly ArraySlice<T> source;
        private readonly ArraySlice<int> mapping;

        public MappedArraySlice(ArraySlice<T> source, ArraySlice<int> mapping)
        {
            this.source = source;
        ...
        public ref T this[int index] => ref this.source[this.mapping[index]];
    }

    internal readonly struct ReadonlyMappedArraySlice<T>
    {
        private readonly ReadOnlyArraySlice<T> source;
        private readonly ArraySlice<int> mapping;

        public ReadonlyMappedArraySlice(ReadOnlyArraySlice<T> source, ArraySlice<int> mapping)
        {
            this.source = source;
            this.mapping = mapping;
        }
        ...
        public T this[int index] => this.source[this.mapping[index]];
    }
}
...
namespace FrankenTui.Text.BidiInternals
{
    ...
    /// </summary>
    internal enum BidiCharacterType
    {
        // Strong Types
