...
internal static class ShowcaseSurface
{
    ...
    private static IWidget BuildContent(ShowcaseDemoState state) =>
        ...
        {
            "guided_tour" => Screen01GuidedTour.Build(state),
            "dashboard" => Screen02Dashboard.Build(state),
            "shakespeare" => Screen03Shakespeare.Build(state),
            "code_explorer" => Screen04CodeExplorer.Build(state),
            "widget_gallery" => Screen05WidgetGallery.Build(state),
            "layout_lab" => Screen06LayoutLab.Build(state),
            "forms_input" => Screen07FormsInput.Build(state),
            "data_viz" => Screen08DataViz.Build(state),
            ...
            "mermaid_showcase" => Screen16Mermaid.Build(state),
            "mermaid_mega_showcase" => Screen17MermaidMega.Build(state),
            "visual_effects" => Screen18VisualEffects.Build(state),
    ...
    private static IWidget BuildAsyncTasks(ShowcaseDemoState state)
    {
        ...
        var details = Panel(
            focusedPanel == 2 ? "Task Details [focus]" : "Task Details",
        ...
        var activity = Panel(
            focusedPanel == 3 ? "Activity [focus]" : "Activity",
        ...
        return new StackWidget(
            ...
            [
                ...
                (LayoutConstraint.Fixed(1), new ParagraphWidget($"n:spawn c:cancel s:scheduler a:aging | focused={focusedPanel} aging={(aging ? "on" : "off")} | bounded_concurrency scheduling_decision cancellation_decision"))
            ]);
    }
